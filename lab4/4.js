...
const Data = {
    ...
    setDependentGeomParameters: function () {
        Camera.r_0 = Math.sqrt(
            Math.pow(1.0 / 2.0, 2) + Math.pow(1.0 / 2.0, 2) + Math.pow(0.3, 2)
        )
    ...
    yt_x: function (x, t) {
        //\C2\C5\D0\CD\D3\D2\DC \CF\D0\CE\C8\C7\C2\CE\C4\CD\D3\DE dyt/dx
        if (x == 0) {
            return 5 * t * (0.2969 * 999999999999999 - 0.126)
        }
        return (
            5 *
            t *
            ((0.2969 * 1) / (2 * Math.sqrt(x)) -
                0.126 -
                0.3516 * x * 2 +
                0.2843 * 3 * x ** 2 -
                0.1015 * 4 * x ** 3)
    ...
    c1_u: function (u, pt) {
        ...
        //\D0\C0\D1\D1\D7\C8\D2\C0\D2\DC \CF\D0\CE\C8\C7\C2\CE\C4\CD\D3\DE dc1/du
        pt.x = x_u
        pt.y = y_u
        pt.z = 0.0
    },
    ...
    generateBoundaryCurves: function (n) {
        ...
        if (this.drawRuledSurfaces) this.calculateRuledSurface()
    ...
    calculateRuledSurface: function () {
        ...
        du = 1 / (N - 1)
        dv = 1 / (M - 1)
        //// \C4\CE\C1\C0\C2\C8\D2\DC \CA\CE\C4 \D0\C0\D1\D7\C5\D2\C0 \D2\CE\D7\C5\CA \CB\C8\CD\C5\C9\D7\C0\D2\CE\C9 \CF\CE\C2\C5\D0\D5\CD\CE\D1\D2\C8
        for (i = 0; i < N; i++) {
            u = du * i
            for (j = 0; j < M; j++) {
                this.c1(u, pt1)
                ...
                v = dv * j
                const x = (1 - v) * pt1.x + v * pt2.x
                const y = (1 - v) * pt1.y + v * pt2.y
                const z = (1 - v) * pt1.z + v * pt2.z
                ...
                const x_u = (1 - v) * pt1_u.x + v * pt2_u.x
                const y_u = (1 - v) * pt1_u.y + v * pt2_u.y
                const z_u = (1 - v) * pt1_u.z + v * pt2_u.z

                const x_v = -pt1.x + pt2.x
                const y_v = -pt1.y + pt2.y
                const z_v = -pt1.z + pt2.z
        ...
        for (i = 0; i < N; i++)
            for (j = 0; j < M; j++) {
                this.verticesSurface[(i * M + j) * 6] =
                    this.pointsSurface[i][j].x
                this.verticesSurface[(i * M + j) * 6 + 1] =
                    this.pointsSurface[i][j].y
                this.verticesSurface[(i * M + j) * 6 + 2] =
                    this.pointsSurface[i][j].z
                this.verticesSurface[(i * M + j) * 6 + 3] =
                    this.normalsSurface[i][j][0]
                this.verticesSurface[(i * M + j) * 6 + 4] =
                    this.normalsSurface[i][j][1]
                this.verticesSurface[(i * M + j) * 6 + 5] =
